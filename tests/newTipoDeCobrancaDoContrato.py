...
from ORM_IXC.context.contextModels.tipoDeCobrancaDoContrato import TipoDeCobrancaDoContrato
...
login = TipoDeCobrancaDoContrato(manager)
query = select(login)\
            .where(TipoDeCobrancaDoContratoModel.id  > 0)\
            .limit(500)\
            .order_by("id")\
            .execute()
